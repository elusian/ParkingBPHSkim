...
// Constructors --
//----------------
BPHDecayMomentum::BPHDecayMomentum( int daugNum, int compNum ):
 oldMom( true ) {
...
const reco::Candidate* BPHDecayMomentum::getDaug(
  ...
  if ( pos != string::npos ) {
    ...
    return ( comp == nullptr ? nullptr :
                               comp->getDaug( name.substr( pos + 1 ) ) );
  }
  map<const string,
...
BPHRecoConstCandPtr BPHDecayMomentum::getComp( const string& name ) const {
  ...
  if ( pos != string::npos ) {
    const BPHRecoCandidate* comp = getComp( name.substr( 0, pos ) ).get();
    return ( comp == nullptr ? nullptr :
                               comp->getComp( name.substr( pos + 1 ) ) );
  }
  map<const string,
...
void BPHDecayMomentum::sumMomentum(
                       const vector<const reco::Candidate*>& dl,
                       const vector<string                >& dn ) const {
  // add the particles to pat::CompositeCandidate
  int n = dl.size();
  while ( n-- ) compCand.addDaughter( *dl[n], dn[n] );
  return;
}


void BPHDecayMomentum::fillDaug( vector<const reco::Candidate*>& ad,
                                 const string& name,
                                 vector<string                >& an ) const {
