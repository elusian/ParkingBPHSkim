...
BPHWriteSpecificDecay::BPHWriteSpecificDecay( const edm::ParameterSet& ps ) {
  ...
  if ( useGP ) consume< vector<pat::GenericParticle        > >( gpCandsToken,
                                                                gpCandsLabel );
  if ( useK0 ) consume< vector<reco::VertexCompositeCandidate> >
                                                              ( k0CandsToken,
                                                                k0CandsLabel );
  if ( useL0 ) consume< vector<reco::VertexCompositeCandidate> >
                                                              ( l0CandsToken,
                                                                l0CandsLabel );
  if ( useKS ) consume< vector<reco::VertexCompositePtrCandidate> >
                                                              ( kSCandsToken,
                                                                kSCandsLabel );
  if ( useLS ) consume< vector<reco::VertexCompositePtrCandidate> >
                                                              ( lSCandsToken,
                                                                lSCandsLabel );
  ...
  if ( writeLambda0 ) produces<pat::CompositeCandidateCollection>(    l0Name );
...
void BPHWriteSpecificDecay::produce( edm::Event& ev,
  ...
  if ( writeLambda0 ) write( ev, lL0   ,    l0Name );
  ...
  if ( k0s != nullptr ) {
    ...
    if ( rIter != rIend ) {
      ...
      while ( pIter != pIend ) {
        ...
        double  pv = pEntry.second;
        switch( id ) {
        case ptMin      : k0s->setPtMin  ( pv ); break;
        case etaMax     : k0s->setEtaMax ( pv ); break;
        case massMin    : k0s->setMassMin( pv ); break;
        case massMax    : k0s->setMassMax( pv ); break;
        case probMin    : k0s->setProbMin( pv ); break;
        case writeCandidate: writeK0s =  ( pv > 0 ); break;
        default: break;
        }
      }
    }
  ...
  if ( l0s != nullptr ) {
    ...
    if ( rIter != rIend ) {
      ...
      while ( pIter != pIend ) {
        ...
        double  pv = pEntry.second;
        switch( id ) {
        case ptMin      : l0s->setPtMin  ( pv ); break;
        case etaMax     : l0s->setEtaMax ( pv ); break;
        case massMin    : l0s->setMassMin( pv ); break;
        case massMax    : l0s->setMassMax( pv ); break;
        case probMin    : l0s->setProbMin( pv ); break;
        case writeCandidate: writeLambda0 = ( pv > 0 ); break;
        default: break;
        }
      }
    }
  ...
  if ( recoB0 && jPsiFound && k0Found ) {
    ...
    if ( rIter != rIend ) {
      ...
      while ( pIter != pIend ) {
        ...
        double  pv = pEntry.second;
        switch( id ) {
        case mPsiMin    : b0->setJPsiMassMin( pv ); break;
        case mPsiMax    : b0->setJPsiMassMax( pv ); break;
        case mK0sMin    : b0->setK0MassMin  ( pv ); break;
        case mK0sMax    : b0->setK0MassMax  ( pv ); break;
        case massMin    : b0->setMassMin    ( pv ); break;
        case massMax    : b0->setMassMax    ( pv ); break;
        case probMin    : b0->setProbMin    ( pv ); break;
        case mFitMin    : b0->setMassFitMin ( pv ); break;
        case mFitMax    : b0->setMassFitMax ( pv ); break;
        case constrMJPsi: b0->setConstr     ( pv > 0 ); break;
        case writeCandidate: writeB0 =      ( pv > 0 ); break;
        default: break;
        }
      }
    }
  ...
  if ( recoLambdab && jPsiFound && l0Found ) {
    ...
    if ( rIter != rIend ) {
      ...
      while ( pIter != pIend ) {
        ...
        double  pv = pEntry.second;
        switch( id ) {
        case mPsiMin    : lb->setJPsiMassMin   ( pv ); break;
        case mPsiMax    : lb->setJPsiMassMax   ( pv ); break;
        case mLambda0Min: lb->setLambda0MassMin( pv ); break;
        case mLambda0Max: lb->setLambda0MassMax( pv ); break;
        case massMin    : lb->setMassMin       ( pv ); break;
        case massMax    : lb->setMassMax       ( pv ); break;
        case probMin    : lb->setProbMin       ( pv ); break;
        case mFitMin    : lb->setMassFitMin    ( pv ); break;
        case mFitMax    : lb->setMassFitMax    ( pv ); break;
        case constrMJPsi: lb->setConstr        ( pv > 0 ); break;
        case writeCandidate: writeLambdab =    ( pv > 0 ); break;
        default: break;
        }
      }
    }
  ...
  if ( bc != nullptr ) {
    ...
    if ( rIter != rIend ) {
      ...
      while ( pIter != pIend ) {
        ...
        double  pv = pEntry.second;
        switch( id ) {
        case ptMin      : bc->setPiPtMin    ( pv ); break;
        case etaMax     : bc->setPiEtaMax   ( pv ); break;
        case mPsiMin    : bc->setJPsiMassMin( pv ); break;
        case mPsiMax    : bc->setJPsiMassMax( pv ); break;
        case massMin    : bc->setMassMin    ( pv ); break;
        case massMax    : bc->setMassMax    ( pv ); break;
        case probMin    : bc->setProbMin    ( pv ); break;
        case mFitMin    : bc->setMassFitMin ( pv ); break;
        case mFitMax    : bc->setMassFitMax ( pv ); break;
        case constrMJPsi: bc->setConstr     ( pv > 0 ); break;
        case writeCandidate: writeBc =      ( pv > 0 ); break;
        default: break;
        }
      }
    }
  ...
  if ( x3872 != nullptr ) {
    ...
    if ( rIter != rIend ) {
      ...
      while ( pIter != pIend ) {
        ...
        double  pv = pEntry.second;
        switch( id ) {
        case ptMin      : x3872->setPiPtMin    ( pv ); break;
        case etaMax     : x3872->setPiEtaMax   ( pv ); break;
        case mPsiMin    : x3872->setJPsiMassMin( pv ); break;
        case mPsiMax    : x3872->setJPsiMassMax( pv ); break;
        case massMin    : x3872->setMassMin    ( pv ); break;
        case massMax    : x3872->setMassMax    ( pv ); break;
        case probMin    : x3872->setProbMin    ( pv ); break;
        case mFitMin    : x3872->setMassFitMin ( pv ); break;
        case mFitMax    : x3872->setMassFitMax ( pv ); break;
        case constrMJPsi: x3872->setConstr     ( pv > 0 ); break;
        case writeCandidate: writeX3872 =      ( pv > 0 ); break;
        default: break;
        }
      }
    }
