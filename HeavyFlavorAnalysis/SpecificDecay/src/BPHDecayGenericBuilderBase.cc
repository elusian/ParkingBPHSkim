...
BPHDecayGenericBuilderBase::BPHDecayGenericBuilderBase():
 ...
 mFitSel( nullptr ),
 lxySigSel( new BPHLxySigSelect( -1.0 ) ),
 minPDiff( 1.0e-4 ),
 outdated( true ) {
}
...
void BPHDecayGenericBuilderBase::setLxySigMin( double lxySigMin) {
  outdated = true;
  lxySigSel->setLxySigMin( lxySigMin );
  return;
}

void BPHDecayGenericBuilderBase::setBSForLxy( const reco::BeamSpot* bs ) {
  outdated = true;
  lxySigSel->setBS( bs );
  return;
}
