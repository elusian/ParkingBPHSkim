...
BPHDecayToV0DiffMassBuilder::BPHDecayToV0DiffMassBuilder(
 ...
 BPHDecayToV0Builder( es, daug1Name, daug2Name,
                          daug1Collection, daug2Collection ),
 BPHDecayToTkpTknSymChargeBuilder( es,
                                   daug1Name, daug1Mass, daug1Sigma,
                                   daug2Name, daug2Mass, daug2Sigma,
                                   daug1Collection, daug2Collection,
                                   expectedMass ),
 p1Mass ( daug1Mass  ),
 p2Mass ( daug2Mass  ),
...
BPHDecayToV0DiffMassBuilder::BPHDecayToV0DiffMassBuilder(
               ...
               double expectedMass, const string& searchList ):
 BPHDecayGenericBuilderBase( es ),
 BPHDecayToV0Builder( es, daug1Name, daug2Name, v0Collection, searchList ),
 BPHDecayToTkpTknSymChargeBuilder( es,
                                   daug1Name, daug1Mass, daug1Sigma,
                                   daug2Name, daug2Mass, daug2Sigma,
                                   nullptr, nullptr, expectedMass ),
 p1Mass ( daug1Mass  ),
 p2Mass ( daug2Mass  ),
 p1Sigma( daug1Sigma ),
 p2Sigma( daug2Sigma ),
...
BPHPlusMinusCandidatePtr BPHDecayToV0DiffMassBuilder::buildCandidate(
  ...
  if ( ( m1 > mcut ) && ( m2 < mcut ) ) return candX;
  if ( ( m1 < mcut ) && ( m2 > mcut ) ) return candY;
  // choose combination having the best invariant mass
  return ( fabs( mv0 - cptrX->mass() ) <
           fabs( mv0 - cptrY->mass() ) ? candX : candY );
}
