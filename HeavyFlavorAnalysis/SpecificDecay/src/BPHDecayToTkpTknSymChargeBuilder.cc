...
void BPHDecayToTkpTknSymChargeBuilder::addParticle(
     const BPHRecoBuilder::BPHGenericCollection* collection,
     int charge, vector<Particle*>& list ) {
  int i;
  ...
  for ( i = 0; i < n; ++i ) {
    ...
    double e1 = ( ( p4.pt () >= pt1Min ) && ( p4.eta() <= eta1Max ) ?
                  sqrt( p2 + ( d1Mass * d1Mass ) ) : -1.0 );
    double e2 = ( ( p4.pt () >= pt2Min ) && ( p4.eta() <= eta2Max ) ?
                  sqrt( p2 + ( d2Mass * d2Mass ) ) : -1.0 );
    if ( ( e1 > 0.0 ) || ( e2 > 0.0 ) ) list.push_back( new Particle( &cand,
                                             tk, px, py, pz, e1, e2 ) );
  }
  return;
}
