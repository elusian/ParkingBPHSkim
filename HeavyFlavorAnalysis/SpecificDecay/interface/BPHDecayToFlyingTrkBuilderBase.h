#ifndef HeavyFlavorAnalysis_SpecificDecay_BPHDecayToFlyingTrkBuilderBase_h
#define HeavyFlavorAnalysis_SpecificDecay_BPHDecayToFlyingTrkBuilderBase_h
/** \class BPHDecayToFlyingTrkBuilderBase
 *
 *  Description: 
...
class BPHDecayToFlyingTrkBuilderBase:
 ...
 public:
  ...
  /** Constructor
   */
  BPHDecayToFlyingTrkBuilderBase( const BPHEventSetupWrapper& es,
      const std::string& flyingName, double flyingMass, double flyingWidth,
      const std::string& trkName, double trkMass, double trkSigma,
      const BPHRecoBuilder::BPHGenericCollection* trkCollection );

  // deleted copy constructor and assignment operator
  BPHDecayToFlyingTrkBuilderBase           ( const BPHDecayToFlyingTrkBuilderBase& x )
                                          = delete;
  BPHDecayToFlyingTrkBuilderBase& operator=( const BPHDecayToFlyingTrkBuilderBase& x )
                                          = delete;

  /** Destructor
   */
  ~BPHDecayToFlyingTrkBuilderBase() override;
 ...
 protected:
  ...
  BPHDecayToFlyingTrkBuilderBase(
      const std::string& trkName, double trkMass, double trkSigma,
      const BPHRecoBuilder::BPHGenericCollection* trkCollection );
 ...
 private:
  ...
  static void filter( const std::vector<const reco::Candidate*>* s,
    ...
    d->reserve( n );
    for ( i = 0; i < n; ++i ) {
      if ( f->accept( *s->at( i ) ) ) d->push_back( s->at( i ) );
    }
  }
  void swap( std::vector< const reco::Candidate*>*& l,
