...
class BPHDecayToFlyingCascadeBuilder:
 ...
 public:
  ...
  BPHDecayToFlyingCascadeBuilder           ( const BPHDecayToFlyingCascadeBuilder& x )
                                         = delete;
  BPHDecayToFlyingCascadeBuilder& operator=( const BPHDecayToFlyingCascadeBuilder& x )
                                         = delete;
 ...
 protected:
  ...
  void fitAndFilter( std::vector<prod_ptr>& prodList ) {
    ...
    for ( iRec = 0; iRec < nRec; ++iRec ) {
      ...
      if ( (   mFitSel->getMassMax() >= 0 ) &&
           ( ! mFitSel->accept( *cptr ) ) ) continue;
      const RefCountedKinematicVertex tdv = cptr->topDecayVertex();
      if ( ( kfChi2Sel->getProbMin() >= 0 ) &&
            !kfChi2Sel->accept( *cptr )     ) continue;
