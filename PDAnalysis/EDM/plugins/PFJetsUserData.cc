...
PFJetUserData::PFJetUserData(const edm::ParameterSet &pSet){
  ...
  consume< reco::VertexCollection >( gt_recVtxs, edm::InputTag( "offlinePrimaryVertices" ) );
  consume< PFJetCollectionAB      >( gt_jetColl, jetLabel_ );
  consume< ValueMap<float>        >( gt_qglMap , qgMap_ );
